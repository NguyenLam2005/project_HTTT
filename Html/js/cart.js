        ...
        <li class="cart__product-item">
            <div class="cart__product-box">
                <div class="cart__product-img-box">
                    <img class="cart__product-img" src="${product.img}" alt="${product.name}">
...
function addToCart() {
    ...
    var cartProduct = products.find(function(product) {
        return product.name.replaceAll(' ', '-').toLowerCase() === productName.toLowerCase();
    });

    if (addCartBtn) {
        addCartBtn.addEventListener('click', function() {
            if (!userAccount[index]) {
                showToast('fail', 'Cảnh báo!', 'Vui lòng đăng nhập để thêm vào giỏ hàng!');
                return;
            }
